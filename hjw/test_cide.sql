...
ALTER TABLE member ADD COLUMN status ENUM('Y', 'N') NOT NULL DEFAULT 'Y';
-- 이메일 인증 후 비밀번호 변경
-- SELECT직후 UPDATE하는 수 ms안에
SELECT * FROM member WHERE email='lee.y2h@email.com';
UPDATE member SET password = 'trewq' WHERE email = 'lee.y2h@email.com';

-- email로 회원 조회 -> 회원 상태를 '탈퇴'로 변경
SELECT * FROM member WHERE email='lee.y2h@email.com';
UPDATE member SET member.status = 'N' WHERE email = 'lee.y2h@email.com';

-- 사용자 휴대폰번호 인증 후 이메일/임시 비밀번호 발급
START TRANSACTION;
SELECT id, email FROM member WHERE phone_num=010-2345-6789 AND status = 'Y' FOR UPDATE;  -- 해당 회원 lock걸고 진행
UPDATE member SET password = '프로그램에서 hash한 임시 비밀번호' WHERE phone_num = 010-2345-6789 AND status = 'Y'; -- 임시 비밀번호 업데이트
COMMIT;

-- 검색어 또는 주소 필터를 적용한 지점 목록 중 원하는 지점 선택  or? and?
SELECT id, name, address FROM place WHERE name LIKE '%짐꽁%' AND address LIKE '%강남%';

-- 선택한 지점의 수강권(10회권, 30회권 등) 구매
SELECT id, count, price FROM membership_option WHERE place_id = 1; -- 지점의 회원권 옵션 조회
START TRANSACTION;
SELECT id, count, price FROM membership_option WHERE id = 2 AND place_id = 1 FOR UPDATE; -- 선택한 수강권 조회
...
BEGIN
    ...
    WHERE p.id = p_place_id                                  -- 지점ID=매개변수로 입력받은 값
      ...
      AND c.capacity >
            (SELECT COUNT(*)
             FROM class_reservation r
             WHERE r.class_id = c.id AND r.cancel_YN = 'N')  -- 예약된 수업이 수업ID와 같고 취소 여부가 NO인 경우
    ORDER BY c.start_time ASC;                               -- 시간 순 정려
END //
DELIMITER ;
